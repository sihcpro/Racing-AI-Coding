/*	Đã nâng cấp kiểu hiện map, nâng cấp thêm dir để xác định đường khi bị kẹt xe
	Dùng dir để in ra bản đồ và bản đồ chính xác
...
public class MainAI11 {
	...
	public MainAI11(Race race, Vector<Car> cars, Car Mycar){
		this.race = race;
		this.Mycar = Mycar;
		this.All_cars = cars;
	...
	final int up= 2, down= 0, right= 1, left= 3;
//					2 la di len
...
///	Point
	...
	int turnNext(Point a, Point b)
	{
		for(int i= 0; i < 4; i++)
		{
			if( ix[i] == b.x - a.x && iy[i] == b.y - a.y )	return i;
		}
		return -1;
	}
	Point wayNext(Point _now, int dir)
	{
		Point _next = new Point( _now.x + ix[dir], _now.y + iy[dir] );
		return _next;
	}
...
///	Matrix
	...
	void setAgain(int c, int r)
	{
		...
		matrix[c][r].upPass();
		if( matrix[c][r].dir() <= 1 && !samePoint(finish, c, r) ){
			matrix[c][r].upPercent(-1);								//	Duong cut
		}
		if( matrix[c][r].dir() == 0 && matrix[c][r].way() != -1 ){
			System.out.println("I had a stuck!");
			this.next = wayNext( this.now, matrix[c][r].way() );
		}
		else{
			this.next = findNext(c, r, this.last);
		}
		matrix[c][r].upWay( turnNext(now, next) );
		this.last = this.now;
		finding= ( matrix[next.x][next.y].pass == 0 );
		if( !finding )
		{
			Point nextNext= findNext(c, r, this.now);
			matrix[next.x][next.y].upWay( turnNext( next, nextNext) );
	...
	void setMatrix(int c, int r)
	{
		if( matrix[c][r].pass() == 0 )
			...
			setAgain(c, r);
//		if( matrix[c][r].way() != -1 )
//			System.out.println( matrix[c][r].way() + " " + ix[ matrix[c][r].way() ] + " " + iy[ matrix[c][r].way() ] );
	}
		...
		for(int r= 0; r < this.race.BlockRow(); r++)
		{
			for(int c= 0; c < this.race.BlockColumn(); c++)
				if( matrix[c][r].map() == ' ' )
					...
					System.out.printf( "%2c",way[ matrix[c][r].way() + 1]);
			System.out.println();
		}
	}
